...
@implementation TCRecommendTestProductView {
    UILabel *_titleLabel;
...
- (instancetype)initWithTitleText:(NSString *)titleText participantsCount:(NSInteger)count iconImageURL:(NSURL *)imageURL alreadyPurchase:(BOOL)purchased presentPrice:(CGFloat)presentPrice originPrice:(CGFloat)originPrice {
    ...
    [self setupSubviews];
    [self setTitleText:titleText participantsCount:count iconImageURL:imageURL alreadyPurchase:purchased presentPrice:presentPrice originPrice:originPrice ];
    self.userInteractionEnabled = YES;
    UITapGestureRecognizer *tapGesturRecognizer=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(productClick)];
...
- (void)setupSubviews {
     UIView *container = [UIView new];
       ...
       [container mas_makeConstraints:^(MASConstraintMaker *make) {
           make.edges.mas_equalTo(UIEdgeInsetsMake(STWidth(5), STWidth(0), STWidth(5), STWidth(0)));
       }];
       ...
       [_iconImageView mas_makeConstraints:^(MASConstraintMaker *make) {
           ...
           make.width.equalTo(_iconImageView.mas_height).multipliedBy(1.5);
