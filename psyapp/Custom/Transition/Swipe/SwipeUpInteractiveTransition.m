...
- (void)panGestureHandler:(UIPanGestureRecognizer *)gesture {
    ...
    switch (gesture.state) {
        ...
        // 当前触控点的赋值
            ...
            CGFloat fraction = (translation.y / mKeyWindow.bounds.size.height);
            // 保证范围
            ...
            dispatch_source_set_event_handler(timer, ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (!_isNeedDismiss && (!_shouldComplete || gesture.state == UIGestureRecognizerStateCancelled)) {
                        ...
                        // 模拟上滑行为
                            CGFloat fraction = (self.oldTranslation.y / mKeyWindow.bounds.size.height);
                            fraction = fmin(fmaxf(fraction, 0.0), 1.0);
                        ...
                        if (self.oldTranslation.y > mKeyWindow.bounds.size.height) {
                        // 当滑出当前屏幕,执行完成动画
