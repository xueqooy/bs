...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view
{
    ...
    [pickerView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.frame.size.height <=1) {
            obj.backgroundColor = [UIColor colorWithRed:212/255.0 green:212/255.0 blue:214/255.0 alpha:1.0];
            CGRect tempRect = obj.frame;
            CGFloat lineW = self.contentView.st_width - 100;
            obj.frame = CGRectMake((pickerView.frame.size.width - lineW) * 0.5, tempRect.origin.y, lineW, 0.5);
        }
    }];
